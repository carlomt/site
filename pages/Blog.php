...
require_once("./template/third_party/htmlpurifier/library/HTMLPurifier.auto.php");
...
if(!empty($_GET['blogaction']))
...
else
    {
        ...
        if($config['debug'])
            {
                echo "first post: ".$iStartingPost;
                echo " number of files: ".sizeof($files);
                echo "<br /> post per page: ".$postsPerPage;
                echo " difference: ".($postsPerPage-$iStartingPost);
            }
        ...
        if(sizeof($files) > $postsPerPage && ($postsPerPage-$iStartingPost) > 0)
            {
                ...
                echo "<a href=\"?action=Blog&firstpost=".$tmp."\">";
            }
        else
            {
        ...
        echo "<span aria-hidden=\"true\">&larr;</span> Older posts</a></li>\n";
        ...
        echo "Newer posts <span aria-hidden=\"true\">&rarr;</span></a></li>\n";
